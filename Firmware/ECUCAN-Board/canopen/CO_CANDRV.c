...
int _CANSetBitRate(void)
{
    unsigned long busspeed = 1000000; //default buspeed in mbps
    unsigned long _fosc = _XTAL_FREQ;
    unsigned char TQ = 0;    
    unsigned char min_s2 = 0;
    unsigned char prop_seg = 1;
	...
	{
		case 0: busspeed = 1000000; break; /* 1 mbit/s */
		case 1: busspeed = 800000; break;  /* 800 kbit/s */
		case 2: busspeed = 500000; break;  /* 500 kbit/s */
		case 3: busspeed = 500000; break;  /* 500 kbit/s */
		case 4: busspeed = 250000; break;  /* 250 kbit/s */
		case 5: busspeed = 125000; break;  /* 125 kbit/s */
		case 6: busspeed = 50000; break;   /* 50 kbit/s */
		case 7: busspeed = 20000; break;   /* 20 kbit/s */
		case 8: busspeed = 10000; break;   /* 10 kbit/s */
    }
    
    //calculate ideal number of TQ per NBT based on bus rate and fosc
    //max time quanta per bit can go up to 25TQ. however that would lower sample point so only up to 20TQ will be accepted here
    for (char i=8; i<=20; i++) {
    ...
    //If no suitable TQ was found for this fosc and busrate    
    if (TQ==0) {
        return 1; 
    }
    
    //Assume 1TQ for sync and max TQ (8) for both prop_seg and seg1
    for (unsigned char j=(unsigned char) (TQ*0.9); j>(unsigned char) (TQ*0.8); j--) {
        if ((TQ-j)<2) { continue; }
        if (((TQ-j)+8+8+1)>=TQ) { 
            min_s2 =  (unsigned char) (TQ-j); 
            break; 
        }
    }

        
    //If no suitable SEG2 was found for this fosc and busrate    
    if (min_s2==0) {
        return 1; 
    }
