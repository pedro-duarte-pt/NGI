...
void _CO_COMM_NMTE_Open(void)
{
	...
	_uNMTE_COBID.word = (unsigned) 0x700L + _uCO_nodeID.byte;
...
{
	{
		...
		*(unsigned long *)(mCANGetPtrTxCOB()) = _uNMTE_COBID.word;
		...
		if (!NMTE_BOOT_SERVICE)
		{
			...
			// Set the data
			if (COMM_STATE_STOP) _uNMTELocalState.byte = _uNMTELocalState.byte | (unsigned) 4;
			else
			if (COMM_STATE_OPER) _uNMTELocalState.byte = _uNMTELocalState.byte | (unsigned) 5;
			else
			if (COMM_STATE_PREOP) _uNMTELocalState.byte = _uNMTELocalState.byte | (unsigned) 127;
